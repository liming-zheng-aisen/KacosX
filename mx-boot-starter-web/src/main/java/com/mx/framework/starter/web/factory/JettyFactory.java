package com.mx.framework.starter.web.factory;
...
import com.mx.framework.mvc.dispatcher.DispatchedServlet;
...
import com.mx.framework.starter.web.handlle.StaticSourceHandle;
import com.mx.framework.starter.web.handlle.WebSocketHandlerSuper;
...
public class JettyFactory {
    ...
    private final static  String THREAD_NAME="MX-Boot";
    ...
    public static Server createServer(JettyConfig config, Class main) throws Exception {
        ...
        if (host.equals("localhost")){
            ServerConnector http2 = initConnectory(server, IpUtil.currentIP(true),config.getPort(),config.getIdleTimeout());
        ...
        // WebSocket启用
        Set<BeanDefinition> set = BeanManager.getBeanDefinitionsByAnnotation(ServerEndpoint.class);
        WebSocketHandlerSuper.registers(set);
        ContextHandler contextHandler = new ContextHandler();
        contextHandler.setContextPath("/ws");
        contextHandler.setHandler(new WebSocketHandlerSuper());
    ...
    private static void setServletHandler(ServletHandler servletHandler){
        Set<ServletBean> servletBeans= ServletBeanInitManager.getServletBeans();
